    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6">
                ...
                <form method="POST" action="{{ route('admin.absensi.export.pdf.daily') }}" class="mt-2">
                    @csrf
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6">
                ...
                <form id="periodeForm" method="POST" action="{{ route('admin.absensi.export.period') }}">
                    @csrf
                    <div class="mb-4">
                ...
                <form id="periodePdfForm" method="POST" action="{{ route('admin.absensi.export.pdf.period') }}" class="mt-2">
                    @csrf
                    <input type="hidden" name="start_date" value="{{ now()->subDays(7)->toDateString() }}">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6">
                ...
                <form id="summaryForm" method="POST" action="{{ route('admin.absensi.export.summary') }}">
                    @csrf
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-slate-700">Tanggal Mulai</label>
                ...
                <form id="summaryPdfForm" method="POST" action="{{ route('admin.absensi.export.pdf.summary') }}" class="mt-2">
                    @csrf
                    <input type="hidden" name="start_date" value="{{ now()->subDays(30)->toDateString() }}">
                    <input type="hidden" name="end_date" value="{{ now()->toDateString() }}">
                    <button type="submit" class="btn btn-secondary w-full">Download PDF</button>
                </form>
            </div>
