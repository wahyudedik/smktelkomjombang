                ...
                <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="excerpt" rows="2" maxlength="500"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $berita->excerpt) }}</textarea>
                        @error('excerpt')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="status"
                                    ...
                                    <option value="archived" {{ old('status', $berita->status) === 'archived' ? 'selected' : '' }}>
                                        Arsip
                                    </option>
                                </select>
                                @error('status')
                            ...
                            @if ($berita->published_at)
                                <p class="text-xs text-slate-500">
                                    ...
                                    Dipublish: {{ $berita->published_at->format('d M Y H:i') }}
                                </p>
                            @endif
                        </div>
                    </div>
