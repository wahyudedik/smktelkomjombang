    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-600 mt-1">Perbarui data kegiatan: <strong>{{ $event->title }}</strong></p>
            </div>
                ...
                <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-5">
                        ...
                        <select name="status"
                            ...
                            <option value="active" {{ old('status', $event->status) === 'active' ? 'selected' : '' }}>Aktif (tampil di landing page)</option>
                            <option value="inactive" {{ old('status', $event->status) === 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                            <option value="archived" {{ old('status', $event->status) === 'archived' ? 'selected' : '' }}>Arsip</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
