<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">{{ __('common.import_calon_osis') }}</h1>
                <p class="text-slate-600 mt-1">{{ __('common.import_calon_description') }}</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-xl border border-slate-200 p-8">
            ...
            <!-- Instructions -->
            <div class="mb-8">
                <h3 class="text-lg font-semibold text-slate-900 mb-4">{{ __('common.import_calon_guide') }}</h3>
            ...
            <form method="POST" action="{{ route('admin.osis.calon.processImport') }}" enctype="multipart/form-data"
                ...
                <div class="bg-slate-50 rounded-lg p-4">
                    <h4 class="text-sm font-medium text-slate-900 mb-2">{{ __('common.file_format_required') }}</h4>
                    <div class="text-sm text-slate-600">
                        <p class="mb-2">{{ __('common.excel_columns_info') }}</p>
                        <ul class="list-disc list-inside space-y-1 ml-4">
                            <li><strong>nama_ketua</strong> - {{ __('common.full_name_label') }} {{ __('common.ketua') }} ({{ __('common.required') }})</li>
                            <li><strong>nama_wakil</strong> - {{ __('common.full_name_label') }} {{ __('common.wakil') }} ({{ __('common.no') }})</li>
                            <li><strong>jenis_kelamin</strong> - {{ __('common.laki_laki') }} {{ __('common.or') }} {{ __('common.perempuan') }} ({{ __('common.no') }})</li>
                            <li><strong>visi_misi</strong> - {{ __('common.vision_mission') }} ({{ __('common.no') }})</li>
                            <li><strong>jenis_pencalonan</strong> - {{ __('common.ketua') }}, {{ __('common.wakil') }}, {{ __('common.or') }} {{ __('common.pasangan') }} ({{ __('common.no') }})</li>
                            <li><strong>status_aktif</strong> - 1 ({{ __('common.active') }}) {{ __('common.or') }} 0 ({{ __('common.inactive') }}) ({{ __('common.no') }})</li>
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="validate_data" class="ml-2 text-sm text-slate-700">
                                {{ __('common.validate_before_import') }}
                            </label>
                        </div>
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function handleFile(file) {
                ...
                if (!allowedTypes.includes(file.type) && !allowedExtensions.includes(fileExtension)) {
                    showError('{{ __('common.file_must_excel') }}');
                    fileInput.value = '';
                    return;
                }
                ...
                if (file.size > maxSize) {
                    showError('{{ __('common.file_too_large') }}');
                    fileInput.value = '';
                    return;
                }
                ...
                uploadText.textContent = '{{ __('common.file_ready_upload') }}';
