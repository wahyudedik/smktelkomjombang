    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="p-6 bg-white shadow sm:rounded-lg space-y-6">
                ...
                <form method="POST"
                    action="{{ route('admin.letters.formats.update', ['format' => $letterFormat->id]) }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="type" name="type"
                                ...
                                <option value="out" @selected(old('type', $letterFormat->type) === 'out')>Keluar</option>
                                <option value="in" @selected(old('type', $letterFormat->type) === 'in')>Masuk</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="period_mode" name="period_mode"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <option value="year" @selected(old('period_mode', $letterFormat->period_mode) === 'year')>Tahunan</option>
                                <option value="month" @selected(old('period_mode', $letterFormat->period_mode) === 'month')>Bulanan</option>
                                <option value="all" @selected(old('period_mode', $letterFormat->period_mode) === 'all')>Tidak Reset</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="counter_scope" name="counter_scope"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <option value="global" @selected(old('counter_scope', $letterFormat->counter_scope) === 'global')>Global</option>
                                <option value="unit" @selected(old('counter_scope', $letterFormat->counter_scope) === 'unit')>Per Unit</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input id="name" name="name" type="text"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                value="{{ old('name', $letterFormat->name) }}" required
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description"
                            class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                            rows="3">{{ old('description', $letterFormat->description) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="is_active" value="1" id="is_active"
                            ...
                            {{ old('is_active', $letterFormat->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="mt-2 overflow-x-auto">
                            <table class="min-w-full text-sm">
                                ...
                                <tbody id="segments-body" class="divide-y divide-slate-200 bg-white">
                                    @foreach ($letterFormat->segments as $index => $segment)
                                        <tr class="segment-row">
                                            <td class="px-4 py-2">
                                                <select name="segments[{{ $index }}][kind]"
                                                    class="w-full rounded-md border-slate-300 segment-kind text-sm focus:border-blue-500 focus:ring-blue-500">
                                                    <option value="">-- Pilih Tipe --</option>
                                                    <option value="sequence" @selected($segment->type === 'sequence')>Nomor Urut
                                                        (Sequence)
                                                    </option>
                                                    <option value="text" @selected($segment->type === 'text')>Teks Statis
                                                        (Text)</option>
                                                    <option value="unit_code" @selected($segment->type === 'unit_code')>Kode Unit
                                                    </option>
                                                    <option value="day" @selected($segment->type === 'day')>Tanggal
                                                        (01-31)</option>
                                                    <option value="month_roman" @selected($segment->type === 'month_roman')>Bulan
                                                        Romawi (I-XII)</option>
                                                    <option value="month_number" @selected($segment->type === 'month_number')>Bulan
                                                        Angka (01-12)</option>
                                                    <option value="year" @selected($segment->type === 'year')>Tahun (2025)
                                                    </option>
                                                    <option value="year_roman" @selected($segment->type === 'year_roman')>Tahun
                                                        Romawi</option>
                                                </select>
                                            </td>
                                            <td class="px-4 py-2">
                                                <input name="segments[{{ $index }}][value]" type="text"
                                                    value="{{ $segment->value }}"
                                            ...
                                            <td class="px-4 py-2">
                                                <input name="segments[{{ $index }}][padding]" type="number"
                                                    min="0" max="10" value="{{ $segment->padding }}"
